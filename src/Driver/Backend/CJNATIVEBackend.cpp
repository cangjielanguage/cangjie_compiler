...
std::vector<TempFileInfo> CJNATIVEBackend::GeneratePreprocessTools(const std::vector<TempFileInfo>& bitCodeFiles)
{
    ...
    for (const auto& bitCodeFile : bitCodeFiles) {
        ...
        // set output
        // In LTO staticlib builds the opt output is the final product for the legacy
        // bitcode-format flow; with --lto-staticlib-format=native the archive is produced
        // downstream by lld + ar, so the opt output is just an interim and goes to temp.
        TempFileKind kind =
